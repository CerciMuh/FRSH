    ...
    <section className="bg-frsh-cream py-16" id="subscribe">
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="text-center mb-10">
          <h2 className={`text-3xl md:text-4xl font-calvino font-bold text-frsh-green mb-4 ${language === 'ar' ? 'font-[serif]' : ''}`}>
            {t('subscribe_to_frsh')}
          </h2>
          <p className={`text-lg font-labil text-frsh-gray-dark/80 max-w-2xl mx-auto ${language === 'ar' ? 'font-[serif]' : ''}`}>
            {t('subscribe_description')}
          </p>
        </div>
        
        <div className="grid md:grid-cols-2 gap-8 mb-10">
          <Card className={`bg-white border-frsh-gray-dark/10 shadow-sm ${language === 'ar' ? 'text-right' : ''}`}>
            <CardHeader>
              <CardTitle className={`text-frsh-green font-calvino text-2xl ${language === 'ar' ? 'font-[serif]' : ''}`}>
                {t('how_it_works')}
              </CardTitle>
            </CardHeader>
            <CardContent className={`space-y-4 font-labil ${language === 'ar' ? 'font-[serif]' : ''}`}>
          ...
          <Card className={`bg-white border-frsh-gray-dark/10 shadow-sm ${language === 'ar' ? 'text-right' : ''}`}>
            ...
            <CardContent className="flex flex-col items-center justify-center py-8">
              <p className={`text-frsh-gray-dark/80 mb-6 text-center ${language === 'ar' ? 'font-[serif]' : ''}`}>
                {t('ready_to_start')}
              </p>
              <a 
              ...
              >
                <Button 
                  className={`w-full bg-frsh-yellow hover:bg-frsh-yellow-light text-frsh-gray-dark font-medium px-6 py-6 flex items-center gap-2 justify-center ${language === 'ar' ? 'font-[serif]' : ''}`}
                >
                  <Calendar className="h-5 w-5" />
                  {t('start_my_subscription')}
                </Button>
              </a>
            </CardContent>
            <CardFooter className={`flex flex-col ${language === 'ar' ? 'items-end' : 'items-start'} pt-4`}>
              <p className={`text-sm text-frsh-gray-dark/70 mb-2 ${language === 'ar' ? 'font-[serif]' : ''}`}>
                {t('or_download_app')}
              </p>
              <a
        ...
        <div className="mt-8 text-center">
          <p className={`text-sm text-frsh-gray-dark/70 mb-3 font-labil ${language === 'ar' ? 'font-[serif]' : ''}`}>
            {t('payment_methods')}
          </p>
